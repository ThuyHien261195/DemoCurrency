...
class CurrencyRepositoryImpl @Inject constructor(
    ...
    override fun getAllGroupInfos(): Flow<List<CurrencyGroupModel>> {
        return currencyDao.getAllGroups().map { data ->
            data.map { it.toCurrencyGroupModel() }
        }
    }

    override fun getGroupInfo(groupId: Int?): Flow<CurrencyGroupModel?> {
        return currencyDao.getGroupInfo(groupId = groupId).map {
    ...
    override fun getCurrenciesByGroup(groupId: Int?): Flow<List<CurrencyInfoModel>> {
        return currencyDao.getCurrenciesByGroup(groupId).map { data ->
    ...
    private suspend fun loadDataFromJson(fileName: String): List<CurrencyInfoModel> {
    ...
    override fun searchCurrencies(keyword: String, groupId: Int?): Flow<List<CurrencyInfoModel>> {
        return currencyDao.searchCurrencies(keyword, groupId).map { data ->
            data.map { it.toCurrencyInfoModel() }
        }
    }
